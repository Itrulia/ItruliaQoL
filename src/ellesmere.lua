...
-------------------------------------------------------------------------------
--  EllesmereUI integration  -- DIFF COPY (ellesmere2)
--
--  This is a scratch copy of ellesmere.lua WITH the "RefreshPage after a
--  discrete edit" change applied, for diffing/debugging against the reverted
--  ellesmere.lua. It is NOT loaded by the addon (not in init.xml). To test it,
--  copy its body over ellesmere.lua -- do not load both files at once, they
--  define the same ItruliaQoL methods and would clash.
--
...
--  an AceConfig options table the way we do for ElvUI. Instead we translate our
--  existing AceConfig option tables (still defined once, per module, in each
--  options.lua) into EllesmereUI widgets at runtime. That keeps a single source
--  of truth -- adding an option to a module lights it up in every host.
--
...
-- AceConfig `disabled` may be a bool or a function; EllesmereUI wants a function.
local function disabledFn(e)
    if e.disabled == nil then return nil end
    return function() return resolve(e.disabled) and true or false end
end

-- >>> RefreshPage change (only difference vs ellesmere.lua) >>>
-- AceConfigDialog re-renders the whole options window after every set -- that is
-- how dependent controls update live (a toggle graying out sub-options, values
-- that mirror one another, enable state, etc.). EllesmereUI does not, so re-run
-- the page after a discrete edit. Only used for discrete controls (toggle /
-- select / execute); sliders and colour swatches write continuously while
-- dragging and rebuilding mid-drag would break the interaction.
local function refreshEUIPage()
...
function ItruliaQoL:ApplyModuleStyles(moduleName)
    ...
    if frame and frame.UpdateStyles then frame:UpdateStyles() end
end
...
-- Sort an args table's keys by AceConfig `order` then name, like AceConfigDialog.
local function orderedKeys(args)
    local list = {}
    for k, v in pairs(args) do
        if type(v) == "table" then list[#list + 1] = k end
    end
    table.sort(list, function(a, b)
        local oa, ob = args[a].order or 100, args[b].order or 100
...
-------------------------------------------------------------------------------
--  AceConfig -> EllesmereUI widget translation
-------------------------------------------------------------------------------
local RenderArgs -- forward declaration

-- Render one AceConfig entry into `parent` at cursor `y`; returns the new cursor.
local function RenderEntry(W, parent, y, key, e)
    if type(e) ~= "table" then return y end
    if e.hidden ~= nil and resolve(e.hidden) then return y end

    local t = e.type
    local _, h

    if t == "group" then
        local nm = resolve(e.name)
        if nm and nm ~= "" and not e.inline then
            _, h = W:SectionHeader(parent, nm, y); y = y - h
        end
        if e.args then y = RenderArgs(W, parent, y, e.args) end
        return y

    elseif t == "description" then
        local txt = resolve(e.name)
        if not txt then return y end
        txt = tostring(txt):gsub("^%s+", ""):gsub("%s+$", "")
        if txt == "" then
            _, h = W:Spacer(parent, y, 10)
        else
            _, h = W:DualRow(parent, y, { text = txt })
        end
        y = y - h
        return y
    ...
    elseif t == "toggle" then
        _, h = W:DualRow(parent, y, {
            type = "toggle", text = resolve(e.name), tooltip = resolve(e.desc),
            disabled = disabledFn(e),
            getValue = function() return e.get() and true or false end,
            setValue = function(v) e.set(nil, v); refreshEUIPage() end,
        })
        y = y - h
        return y
    ...
    elseif t == "range" then
        local mn = e.min or 0
        _, h = W:DualRow(parent, y, {
            type = "slider", text = resolve(e.name), tooltip = resolve(e.desc),
            min = mn, max = e.max or 100, step = e.step or 1,
            disabled = disabledFn(e),
            getValue = function()
                local v = e.get()
            ...
            setValue = function(v) e.set(nil, v) end,
        })
        y = y - h
        return y
    ...
    elseif t == "select" then
        local values, order
        if e.dialogControl == "LSM30_Font" then
            -- The LSM font hashtable is name -> path; show the name + a font
            -- preview instead of the raw path (matches the per-module fonts).
            values, order = ItruliaQoL:EUIFontValues()
        else
            values = resolve(e.values) or {}
            order = selectOrder(values)
        end
        _, h = W:DualRow(parent, y, {
            type = "dropdown", text = resolve(e.name), tooltip = resolve(e.desc),
            values = values, order = order,
            disabled = disabledFn(e),
            getValue = function() return e.get() end,
            setValue = function(v) e.set(nil, v); refreshEUIPage() end,
        })
        y = y - h
        return y
    ...
    elseif t == "color" then
        _, h = W:DualRow(parent, y, {
            type = "colorpicker", text = resolve(e.name), tooltip = resolve(e.desc),
            hasAlpha = e.hasAlpha and true or false,
            getValue = function() return e.get() end,
            setValue = function(r, g, b, a) e.set(nil, r, g, b, a) end,
        })
        y = y - h
        return y
    ...
    elseif t == "input" then
        _, h = W:DualRow(parent, y, {
            type = "input", text = resolve(e.name), tooltip = resolve(e.desc),
            inputWidth = 180,
            getValue = function() return (e.get and e.get()) or "" end,
            setValue = function(txt) e.set(nil, txt) end,
        })
        y = y - h
        return y
    ...
    elseif t == "execute" then
        _, h = W:DualRow(parent, y, {
            type = "button", text = resolve(e.name),
            disabled = disabledFn(e),
            onClick = function() if e.func then e.func() end; refreshEUIPage() end,
        })
        y = y - h
        return y
    end
...
function RenderArgs(W, parent, y, args)
    for _, key in ipairs(orderedKeys(args)) do
        y = RenderEntry(W, parent, y, key, args[key])
    end
    return y
end
...
--
--  The auto-translator above derives the panel from each module's AceConfig
--  table. A module can instead hand-author its EllesmereUI settings by defining
--  Module:GetEUIOptions() in its own options.eui.lua -- returning
--    { name = "Display Name", rows = { <row>, <row>, ... } }
--  When present, that manual list is used INSTEAD of translating the AceConfig
--  tree for that module (see buildModulePage). Rows use clean get/set closures
--  (no AceConfig `(info, value)` wrapper):
--
...
function ItruliaQoL:RenderEUIList(W, parent, y, rows)
    ...
    local function wrap(fn, refresh)
        if not refresh or not fn then return fn end
        return function(...)
            fn(...)
            local EUI = ItruliaQoL.EUI
            if EUI and EUI.RefreshPage then EUI:RefreshPage() end
    ...
    for _, item in ipairs(rows) do
        if item.rows then
            if item.header then _, h = W:SectionHeader(parent, item.header, y); y = y - h end
            y = self:RenderEUIList(W, parent, y, item.rows)
        elseif item.header then
            _, h = W:SectionHeader(parent, item.header, y); y = y - h
        elseif item.spacer then
            _, h = W:Spacer(parent, y, item.spacer); y = y - h
        elseif item.text then
            _, h = W:DualRow(parent, y, { text = item.text }); y = y - h
        elseif item.type == "toggle" then
            _, h = W:DualRow(parent, y, {
                type = "toggle", text = item.label, tooltip = item.tooltip,
                disabled = item.disabled, getValue = item.get, setValue = wrap(item.set, item.refresh),
            }); y = y - h
        elseif item.type == "slider" then
            ...
            local getV = item.get
            _, h = W:DualRow(parent, y, {
                type = "slider", text = item.label, tooltip = item.tooltip,
                min = item.min, max = item.max, step = item.step,
                disabled = item.disabled,
                getValue = function()
                    local v = getV and getV()
                    if v == nil then return item.min or 0 end
                    return v
                end,
                setValue = item.set,
            }); y = y - h
        elseif item.type == "select" then
            ...
            _, h = W:DualRow(parent, y, {
                type = "dropdown", text = item.label, tooltip = item.tooltip,
                values = vals, order = item.order or selectOrder(vals),
                disabled = item.disabled, getValue = item.get, setValue = wrap(item.set, item.refresh),
            }); y = y - h
        elseif item.type == "color" then
            _, h = W:DualRow(parent, y, {
                type = "colorpicker", text = item.label, hasAlpha = item.hasAlpha,
                getValue = item.get, setValue = item.set,
            }); y = y - h
        elseif item.type == "input" then
            _, h = W:DualRow(parent, y, {
                type = "input", text = item.label, tooltip = item.tooltip,
                inputWidth = item.width or 180, disabled = item.disabled,
                getValue = item.get, setValue = wrap(item.set, item.refresh),
            }); y = y - h
        elseif item.type == "execute" then
            _, h = W:DualRow(parent, y, {
                type = "button", text = item.label, disabled = item.disabled, onClick = wrap(item.func, item.refresh),
            }); y = y - h
        end
    end
    return y
...
function ItruliaQoL:EUIFontFamilyRow(f, apply)
    ...
    return {
        ...
        values = vals,
        order = order,
        get = function() return f.fontFamily end,
        set = function(v) f.fontFamily = v; if apply then apply() end end,
    }
end
...
function ItruliaQoL:EUIFontRows(f, apply, exclude)
    ...
    local function add(key, row)
        if not exclude[key] then rows[#rows + 1] = row end
    end

    add("size", { type = "slider", label = "Size", min = 1, max = 68, step = 1,
        get = function() return f.fontSize end,
        set = function(v) f.fontSize = v; apply() end })
    add("font", self:EUIFontFamilyRow(f, apply))
    add("outline", { type = "select", label = "Outline", values = self.OutlineSettings,
        get = function() return f.fontOutline or "NONE" end,
        set = function(v) f.fontOutline = (v ~= "NONE") and v or nil; apply() end })
    add("justify", { type = "select", label = "Justify", values = self.JustifyHSettings,
        get = function() return f.justifyH or "CENTER" end,
        set = function(v) f.justifyH = v; apply() end })
    add("shadowX", { type = "slider", label = "Shadow X Offset", min = -5, max = 5, step = 1,
        get = function() return f.fontShadowXOffset end,
        set = function(v) f.fontShadowXOffset = v; apply() end })
    add("shadowY", { type = "slider", label = "Shadow Y Offset", min = -5, max = 5, step = 1,
        get = function() return f.fontShadowYOffset end,
        set = function(v) f.fontShadowYOffset = v; apply() end })
    add("shadowColor", { type = "color", label = "Shadow Color", hasAlpha = true,
        get = function() local c = f.fontShadowColor return c.r, c.g, c.b, c.a end,
        set = function(r, g, b, a) f.fontShadowColor = { r = r, g = g, b = b, a = a }; apply() end })
    add("strata", { type = "select", label = "Frame Strata", values = self.FrameStrataSettings,
        get = function() return f.frameStrata or "BACKGROUND" end,
        set = function(v) f.frameStrata = v; apply() end })
    add("level", { type = "slider", label = "Frame Level", min = 1, max = 10, step = 1,
        get = function() return f.frameLevel or 1 end,
        set = function(v) f.frameLevel = v; apply() end })
...
StaticPopupDialogs["ITRULIAQOL_EUI_INPUT"] = {
    ...
    OnAccept = function(self)
        ...
        if d and d.onAccept and eb then d.onAccept(eb:GetText()) end
    end,
...
function ItruliaQoL:BuildEUIProfilesPage(parent, yOffset)
    ...
    local function names() return db:GetProfiles() end
    local function currentValues()
        local vals = {}
        for _, n in ipairs(names()) do vals[n] = n end
        return vals
    end
    local function otherValues()
        local vals, cur = {}, db:GetCurrentProfile()
        for _, n in ipairs(names()) do if n ~= cur then vals[n] = n end end
        return vals
    end
    local function sortedKeys(tbl)
        local o = {}
        for k in pairs(tbl) do o[#o + 1] = k end
        table.sort(o)
        return o
    end
    ...
    do
        ...
        _, h = W:DualRow(parent, y, {
            type = "dropdown", text = "Current Profile",
            values = cv, order = sortedKeys(cv),
            getValue = function() return db:GetCurrentProfile() end,
            setValue = function(v) db:SetProfile(v); refresh() end,
        })
        y = y - h
    end

    _, h = W:DualRow(parent, y, {
        type = "button", text = "New Profile",
        onClick = function()
            ShowInputPopup("Enter a name for the new profile:", "", function(name)
                name = name and name:match("^%s*(.-)%s*$") or ""
                if name ~= "" then db:SetProfile(name); refresh() end
            end)
        end,
    })
    y = y - h
    ...
    do
        ...
        _, h = W:DualRow(parent, y,
            { type = "dropdown", text = "Copy From",
              values = ov, order = sortedKeys(ov),
              getValue = function() return self._euiCopyFrom end,
              setValue = function(v) self._euiCopyFrom = v end },
    ...
    do
        ...
        _, h = W:DualRow(parent, y,
            { type = "dropdown", text = "Delete Profile",
              values = ov, order = sortedKeys(ov),
              getValue = function() return self._euiDeleteSel end,
              setValue = function(v) self._euiDeleteSel = v end },
...
function ItruliaQoL:CreateEUIMover(module, frame, moduleName)
    ...
    EUI:RegisterUnlockElements({
        EUI.MakeUnlockElement({
            ...
            group    = "Itrulia",
            order    = 1,
            isHidden = function() return not module.db.enabled end,
            ...
            clearPos = function()
                ...
                if frame.UpdateStyles then frame:UpdateStyles() end
            end,
            applyPos = function()
                if frame.UpdateStyles then frame:UpdateStyles() end
            end,
...
function ItruliaQoL:InjectEUISidebar()
    ...
    for _, group in ipairs(EUI.ADDON_GROUPS) do
        if group.key == "itrulia" then return end
    end
...
function ItruliaQoL:RegisterEUI(parentOptions)
    local EUI = self.EUI
    if not (EUI and EUI.RegisterModule and EUI.Widgets) then return end
    if self._euiRegistered then return end
    ...
    for _, list in pairs(perPage) do
        table.sort(list, function(a, b)
            ...
            if oa == ob then return tostring(resolve(ea.name)) < tostring(resolve(eb.name)) end
            return oa < ob
        end)
    end
    ...
    local function buildModulePage(page)
        return function(parent, yOffset)
            ...
            if #list == 0 then
                _, h = W:DualRow(parent, y, { text = "No settings on this page." }); y = y - h
            end
            for _, key in ipairs(list) do
                ...
                if spec then
                    -- Manual list (options.eui.lua) -- used instead of translating.
                    _, h = W:SectionHeader(parent, spec.name or resolve(grp.name) or key, y); y = y - h
                    y = ItruliaQoL:RenderEUIList(W, parent, y, spec.rows or {})
                else
                    -- Fallback: auto-translate the AceConfig tree.
                    _, h = W:SectionHeader(parent, resolve(grp.name) or key, y); y = y - h
                    if grp.args then y = RenderArgs(W, parent, y, grp.args) end
                end
    ...
    local PAGES = {
        [PAGE_GENERAL] = function(parent, yOffset)
            ...
            -- Hand-authored, from general/options.eui.lua (not auto-translated).
            local spec = ItruliaQoL.GetGeneralEUIOptions and ItruliaQoL:GetGeneralEUIOptions()
            if spec and spec.rows then y = ItruliaQoL:RenderEUIList(W, parent, y, spec.rows) end
            return math.abs(y)
        end,
        ...
        [PAGE_PROFILES]   = function(parent, yOffset) return ItruliaQoL:BuildEUIProfilesPage(parent, yOffset) end,
        [PAGE_IMPEXP]     = function(parent, yOffset) return ItruliaQoL:BuildEUIImportExportPage(parent, yOffset) end,
    }
    ...
    if not ok then
        pcall(function() EUI:RegisterModule(addonName, config) end)
    end
